...
variable "topic-name" {
  type        = string
  default     = "sentinel-cloud-audit-topic"
...
resource "google_pubsub_topic" "sentinel-cloud-audit-topic" {
  count   = "${var.topic-name != "sentinel-cloud-audit-topic" ? 0 : 1}"
  name    = var.topic-name
  project = data.google_project.project.project_id
}
